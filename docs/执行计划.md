...
erDiagram
    ...
    USER ||--o{ NOTE : ""
    USER ||--o{ DICTATION_RECORD : ""
    USER ||--o{ STUDY_SESSION : ""
    USER ||--o{ AI_CONVERSATION : ""
    NOTE ||--o{ DICTATION_RECORD : "被听写"
    ...
    NOTE { string id PK string user_id FK string word string phonetic string meaning string pos string example string example_zh string context text ai_explanation string tags string source bool starred int mastery int review_count int dictation_wrong datetime last_reviewed_at datetime next_review_at datetime created_at datetime updated_at datetime deleted_at "墓碑" }
    DICTATION_RECORD { string id PK string user_id FK string note_id FK string attempt_id text user_answer bool is_correct datetime created_at }
...
### M9 单词记录 —— 背单词核心(2–3 天)
- notes 表扩展:音标 / 释义 / 词性 / 例句(含译文)/ 来源 / 星标 / SRS 字段,并补 `updatedAt` 与 `deletedAt` 墓碑 —— 增量合并与软删除的依据
- 单词本页:增删改查、搜索(词条或释义)、标签与来源筛选、排序(最近更新 / 字母序)、只看待复习、星标、软删除
- SRS 复习:熟悉度 0–5 档驱动间隔(10 分钟 → 1/3/7/15/30 天),答对升档、答错归零并 10 分钟后重排
- 一键入库:阅读透视镜改造为写入结构化字段;写作「词汇升级」与翻译 `vocabulary` 各补一个存词入口
- 仪表盘:生词统计拆为总量 / 待复习 / 已掌握,待复习可跳转筛选
- **验收**:阅读选词存词 → 单词本页编辑并 AI 补全 → 标记认得/忘了 → 仪表盘待复习数变化

### M10 单词听写(2–3 天)
- 新增 `dictationRecords` 表(Dexie v5),沿用 `attemptId` 分组统计每次正确率
- `utils/speech.ts`:封装 Web Speech API —— 语音列表异步就绪、缺英语语音包时超时兜底、首次调用须在手势内、卸载停止朗读
- 听写页三态:设置(范围 / 题量 / 英音美音 / 语速 / 朗读次数 / 提示强度)→ 作答 → 结算
- 判分忽略大小写与连字符;错词累计 `dictationWrong` 并联动 SRS 降档
- **验收**:选一组待复习词 → 听发音拼写 → 提交判分 → 结算页看到正确率与错词

### M11 分发与发布(1 天)
- `.github/workflows/pages.yml`:推送 `main` 即构建并发布 Pages,手机与电脑共用同一网址
- 版本号对齐:`frontend/package.json` 与 `backend` 自报版本统一为 1.1.0
- `.gitignore` 加固:补签名密钥与移动端凭据模式;新增 `.env.example` 模板
- README:新增两个模块说明、网页版地址、跨设备迁移步骤
- **验收**:推送后 CI 与 Pages 工作流均绿;打 `v1.1.0` 后 Release 出现新 APK
