...
**问询条法则 (The Margin-Enquiry Rule).** 全局助教是书页边缘的问询条,不是聊天软件:悬浮钮与透视镜触发钮同族;问答像批注长在纸上(用户句水洗右倾、AI 墨字左对齐);角色决定语气、页面决定上下文,两者都显式注入 system prompt(M6)。

**网关静默回退法则 (The Gateway Fallback Rule).** AI 请求只经一个网关(api/gateway.ts):本地直连或服务器代理由设置决定,后端不可达时静默回落本地、服务器自身错误(503/502)如实上报——回落是容错,不是欺骗(M7)。
